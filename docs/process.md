...
Our requirements are **relatively volatile**. The main goal—providing relevant movie recommendations—is stable, but implementation details may change after experiments. For example, language, preprocessing, and missing-overview behaviour can affect TF-IDF/cosine text ranking, while new accounts naturally have no preference data. Testing may therefore refine the deterministic ranking contract or the imported catalogue data without changing the user-visible need.

### 2. Safety and legal impact

The project has **low safety impact** because incorrect movie recommendations do not threaten health, physical safety, or critical financial operations. However, we must protect account passwords and personal ratings, isolate one account from another, and respect licensing and attribution for movie data. These issues require responsible data handling and basic documentation, but not heavyweight change control.
...
If the team used a **fully plan-driven process**, the biggest risk would be committing too early to unsuitable catalogue and text-ranking assumptions. If preprocessing or missing-overview fallback were frozen before enough testing, the team might later discover irrelevant similar movies or description search results. Changing it could require reworking the importer, ranking contract, API, and interface. The first visible symptom would be poor recommendation relevance during testing while completed components still depend on the original behaviour.
