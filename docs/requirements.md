...
Interview note: the evidence behind these personas is an anonymous
questionnaire with 8 valid responses from university students aged 18 to 20,
recorded in `docs/user-research.md` on 2026-09-17. The follow-up
conversations that C03 is expected to record are not in the repository yet,
so this section has to be checked against them before submission. The quoted
sentences are verbatim free-text answers from the questionnaire, translated
from Vietnamese, and are not attributed to any named person.
...
The rule numbers follow the routes and stories mapped in
`docs/traceability.md`. Each rule is a constraint the system enforces, with a
worked example.

| ID | Rule | Worked example |
|---|---|---|
| BR1 | A viewer selects between 1 and 5 favourite genres. | Choosing Action, Comedy and Drama (3 genres) is accepted. Choosing a sixth genre after 5 is refused and the 5 stay selected. Confirming with 0 genres is refused. |
| BR2 | A recommended movie matches at least one selected genre. | With Action selected, a movie tagged Action and Comedy can be recommended. A movie tagged only Drama cannot. |
| BR3 | A recommendation list has no duplicates and at most 10 movies. | If 12 movies match, 10 are shown. If a movie matches two selected genres, it appears once. |
| BR4 | When nothing matches, the viewer sees a no-match message, up to 10 popular alternatives, and actions to change preferences or retry. | Genres chosen have 0 matching movies, so the page shows the message and 10 popular movies, not an empty list. |
| BR5 | Popular movies are ordered by popularity score from highest to lowest, and equal scores by title from A to Z, with at most 10 shown. | Movie A scores 90, Movie B 80 and Movie C 80. The order is A, B, C. |
| BR6 | Every movie has a unique ID that opens its details page. | A card with ID 42 opens the details of movie 42. An ID that does not exist shows "Movie not found". |
| BR7 | A rating is a whole number from 1 to 5. | 1 and 5 are accepted. 0, 6 and 3.5 are rejected and an earlier rating is kept. |
| BR8 | Saved genres and ratings belong to the current session only. | Session X rates a movie 4. Session Y opens the same movie and does not see a rating. |
| BR9 | A rating of 4 or 5 is a positive ranking signal, 1 or 2 is negative, and 3 is neutral. | Two candidates both score 80. After an Action movie is rated 5, Action ranks above Comedy. After a rating of 1, Comedy ranks above Action. After a rating of 3 with scores of 90 and 80, the order stays as it was. |
| BR10 | The genre filter shows only the chosen genre from the current list and leaves saved preferences unchanged. | A list of 2 Action and 3 Comedy movies shows exactly 2 movies under the Action filter and 5 again once the filter is removed. |
| BR11 | Similar movies share at least one genre with the original, exclude the original, and number at most 5. | A movie has 7 candidates that share a genre. 5 are shown and the original is not among them. |
| BR12 | A title search needs at least 2 characters and shows at most 10 matches. | Searching "I" shows "Enter at least 2 characters". Searching "In" that matches 14 titles shows 10. |
| BR13 | The recommendation explanation has exactly 3 steps. | The page lists 3 steps: choose genres, receive recommendations, optionally rate movies. |
| BR14 | Resetting the profile needs confirmation and clears all saved genres and ratings. | A profile with 3 genres and 2 ratings holds 0 and 0 after confirming. Without confirming, it still holds 3 and 2. |
...
The Story issues do not say where the links to `/search`, `/popular`,
`/about-recommendations` and `/profile` sit. The diagram below reaches them
from the landing route, and the team needs to confirm that choice.

```mermaid
flowchart TD
    HOME["/"] -->|Start| PREF["/preferences"]
    HOME -->|no preferences yet| REC["/recommendations"]
    HOME --> POP["/popular"]
    HOME --> SEARCH["/search"]
    HOME --> ABOUT["/about-recommendations"]
    HOME --> PROFILE["/profile"]
    PREF -->|1 to 5 genres saved| REC
    PREF -->|0 genres or a sixth genre| PREF
    REC -->|Enter preferences| PREF
    REC -->|no match: popular alternatives, edit preferences| PREF
    REC -->|source error: retry| REC
    REC -->|genre filter on or off| REC
    REC -->|View details| MOVIE["/movies/:movieId"]
    POP -->|select a movie| MOVIE
    SEARCH -->|select a result| MOVIE
    MOVIE -->|Similar movies: select one| MOVIE
    MOVIE -->|back to list, preferences kept| REC
    MOVIE -->|invalid ID: link to list| REC
    MOVIE -->|rating 1 to 5 saved, otherwise rejected| MOVIE
    PROFILE -->|confirm reset| PREF
    ABOUT --> HOME
```

Every route in the table appears in the diagram and can be reached from `/`.
The empty state has two exits: a guest with no preferences sees popular
movies on `/recommendations` with an action to open `/preferences`, and a
guest whose genres match nothing sees popular alternatives with actions to
edit preferences or retry.
